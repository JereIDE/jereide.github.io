...
const Nav = styled.nav`
  ...
  pointer-events: none;
  transition:
    background 0.5s cubic-bezier(0.28, 0.11, 0.32, 1),
    padding-top 0.5s cubic-bezier(0.28, 0.11, 0.32, 1);
  padding-top: 0;
  * {
...
const Wrapper = styled.div`
  ...
  transition:
    ...
    max-width 0.5s cubic-bezier(0.28, 0.11, 0.32, 1),
    border-radius 0.5s cubic-bezier(0.28, 0.11, 0.32, 1),
...
const MenuLink = styled(Link)`
  color: ${({ $current }) =>
    $current ? "var(--foreground-color)" : "var(--glyph-gray)"};
  display: inline-block;
  line-height: 22px;
  white-space: nowrap;
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (
        ...
        islandRef?.current &&
        !islandRef.current.contains(event.target)
      ) {
        setIsOpen(false);
      }
    };
